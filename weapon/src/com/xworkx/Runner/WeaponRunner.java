...
import com.xworkx.weaponDTO.WeaponDTO;
...
public class WeaponRunner {
	public static void main(String[] args) {
		...
		List<WeaponDTO> weaponsAfter2024 = weaponSet.stream()
				.filter(w -> w.getManfDate().isAfter(LocalDate.of(2024, 12, 31))).collect(Collectors.toList());
		...
		List<WeaponDTO> weaponsBefore2024 = weaponSet.stream()
				.filter(w -> w.getManfDate().isBefore(LocalDate.of(2024, 1, 1))).collect(Collectors.toList());
		...
		Optional<WeaponDTO> weaponOptional = weaponSet.stream().filter(w -> w.getId() == searchId).findFirst();

		System.out.println("\nWeapon with ID " + searchId + ":");
		// weaponOptional.ifPresentOrElse(System.out::println, () ->
		// System.out.println("Weapon not found."));

		// 5. Sort elements by id in descending order
		List<WeaponDTO> sortedWeapons = weaponSet.stream().sorted(Comparator.comparingInt(WeaponDTO::getId).reversed())
